...
(defn- variable-type-error [variable-name var-type arg-type]
  (format "Variable '$%s' of type '%s' used in position expecting type '%s'."
          variable-name (ve/strip-bang (name var-type)) (ve/strip-bang (name arg-type))))
...
(defn- effective-type
  ...
  (if (and (not default-value) (s/valid? variable-type nil))
    variable-type
    (spec/add-required (namespace variable-type) (name variable-type))))

(defnodevisitor variable-type-mismatch :pre :argument
